...
def get_pypi_download_url(package,version):
 ...
 elif versioninmetadata['releases']:
...
def init_repos(options):
 ...
 if options.top_packages:
  source_directories=tuple(download_and_extract_top_packages(directory=
      options.output,workers=options.workers,limit=slice(None,options.
      top_packages)))
 else:
  source_directories=download_and_extract(package=options.pypi_package,
      version=options.version,directory=options.output),
 for source_directory in source_directories:
  git_create_repository(source_directory)
 if options.black_repoisNone:
...
def format_repo_with_version(repo,from_branch,black_repo,black_version,
 ...
 format_cmd:List[Union[Path,str]]=[black_runner(black_version.version,
     black_repo),(black_repo/'black.py').resolve(),'.']
...
def format_repos(repos,options):
 ...
 for repo in repos:
  ...
  for black_version in black_versions:
   from_branch=format_repo_with_version(repo=repo,from_branch=from_branch,
...
def main():
 ...
 parser.add_argument('-b','--black-repo',help="Black's Git repository.",
     type=Path)
 ...
 parser.add_argument('-i','--input',default=Path('/input'),type=Path,help=
     'Input directory to read configuration.')
 parser.add_argument('-o','--output',default=Path('/output'),type=Path,help
