...
from astor.code_gen import Delimit, precedence_setter
from astor.op_util import Precedence, get_op_precedence, get_op_symbol
from astor.string_repr import pretty_string

set_precedence = precedence_setter()


class FormatterSourceGenerator(SourceGenerator):
    """This visitor is able to transform a well formed syntax tree into Python
    sourcecode. This class is custom so that a lot of indentation can be removed
    """

    using_unicode_literals = False

    def __init__(
        self, indent_with, add_line_information=False, pretty_string=pretty_string
    ):
        self.result = []
        ...
        callable_closure = callable
        isinstance_closure = isinstance
        len_closure = len
        ...
        def write(*params):
            ...
            """
            for item in params:
                if isinstance_closure(item, AST):
                    visit(item)
                elif callable_closure(item):
                    item()
                else:
                    if self.new_lines:
                        append("\n" * self.new_lines)
                        self.colinfo = len_closure(result), 0
    ...
    def __getattr__(
        self, name, defaults={"keywords": (), "_pp": Precedence.highest}.get
    ):
    ...
    def visit_FunctionDef(self, node, is_async=False):
        ...
        self.statement(node, f"{prefix}def {node.name}", "(")
    ...
    def visit_Call(self, node, len=len):
        ...
        set_precedence(Precedence.Comma, *(x.value for x in keywords if x.arg))
            ...
            def recurse(node):
                for value in node.values:
                    if isinstance(value, ast.Str):
                        ...
                        raise AssertionError("Invalid node %s inside JoinedStr" % kind)
    ...
    def _handle_numeric_constant(self, value):
        ...
        if isinstance(x, complex):
            ...
            if x.real == 0:
                ...
                s = "({}{}{})".format(real, ["+", ""][imag.startswith("-")], imag)
        else:
            s = real
        self.write(s)
    ...
    def visit_Dict(self, node):
        with self.delimit("{}"):
            for idx, (key, value) in enumerate(zip(node.keys, node.values)):
                ...
                self.write(
                    "," if idx else "", key if key else "", ":" if key else "**", value
                )
